...
public class Main2048View extends View {

    //Internal Constants
    static final int BASE_ANIMATION_TIME = 100000000;
    private static final String TAG = Main2048View.class.getSimpleName();
    ...
    /**
     * An instance of Main2048Game
     */
    public final Main2048Game game;
    //Internal variables
    ...
    Main2048View(Context context) {
        super(context);

        Resources resources = context.getResources();
        //Loading resources
        game = new Main2048Game(context, this);
        try {
            //Getting assets
    ...
    /**
     * Changes grid size when the view size changes.
     */
    @Override
     ...
     * @param draw an instance of Drawable
     * @param startingX starting x position
     * @param startingY starting y position
     * @param endingX ending x position
     * @param endingY ending y position
     */
    ...
    /**
     * draw out the cell text
     * @param canvas the canvas to draw on
     * @param value the value to know to change the color of cell
     */
    ...
    /**
     * Draws the score boxes and text on the game screen
     */
     ...
     * @param canvas the canvas to draw on
     * @param lightUp whether the button should light up
     */
    ...
    private void drawCells(Canvas canvas) {
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                Block currentBlock = game.grid.getCellContent(xx, yy);
                if (currentBlock != null) {
                    //Get and represent the value of the block
                    ...
                    for (int i = aArray.size() - 1; i >= 0; i--) {
                        ...
                        if (aCell.getAnimationType() == Main2048Game.SPAWN_ANIMATION) {
                            animated = true;
                        }
                        if (!aCell.isActive()) {
                            continue;
                        ...
                        if (aCell.getAnimationType() == Main2048Game.SPAWN_ANIMATION) { // Spawning animation
                        ...
                        } else if (aCell.getAnimationType() == Main2048Game.MERGE_ANIMATION) { // Merging Animation
                            double percentDone = aCell.getPercentageDone();
                            ...
                            bitmapCell[index].setBounds((int) (sX + cellScaleSize), (int) (sY + cellScaleSize), (int) (eX - cellScaleSize), (int) (eY - cellScaleSize));
                            bitmapCell[index].draw(canvas);
                        } else if (aCell.getAnimationType() == Main2048Game.MOVE_ANIMATION) {  // Moving animation
                            double percentDone = aCell.getPercentageDone();
                            int tempIndex = index;
                            if (aArray.size() >= 2) {
    ...
    private void drawEndGameState(Canvas canvas) {
        ...
        for (AnimationCell animation : game.aGrid.globalAnimation) {
            if (animation.getAnimationType() == Main2048Game.FADE_GLOBAL_ANIMATION) {
    ...
    /**
     * draw out the text for endless mode
     * @param canvas the canvas to draw on
     */
    ...
    /**
     * create the end game state
     * @param canvas the canvas to draw on
     * @param win boolean whether won or not
     * @param showButton show button if true
     */
    ...
    /**
     * get all the rectangle cell ids
     * @return the rectangle cell ids
     */
     ...
     * @param height get the height of the layout
     */
    ...
    /**
     * center the text
     * @return the postion where the text should be
     */
